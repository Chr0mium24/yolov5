...
def main():
    ...
    # 验证数据集结构
    print("正在验证数据集结构...")
    train_valid = (input_path / 'train' / 'images').exists() and \
                  (input_path / 'train' / 'labels').exists()
    val_valid = (input_path / 'val' / 'images').exists() and \
                (input_path / 'val' / 'labels').exists()

    if not (train_valid and val_valid):
        print(f"错误: 数据集结构不完整或格式不正确。")
        print(f"期望的目录结构:")
        print(f"- {input_path}/train/images/")
        print(f"- {input_path}/train/labels/")
        print(f"- {input_path}/val/images/")
        print(f"- {input_path}/val/labels/")
        print(f"请确保以上路径都存在。")
        return
